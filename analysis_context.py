...
- 중요한 판단마다 표 번호(표1, 표2, 표3...), 컬럼, 기간과 실제 값을 근거로 적는다. 표 출처를 표시할 때는 D1 같은 영문형 식별자 대신 표1 형식을 사용하되, 실제 데이터 값에 포함된 D1은 변경하지 않는다. 데이터가 부족하면 질문 의도와 짧은 검색 질문을 짝지어 2~3개 제시한다.
...
5. "추가로 찾을 표" data-table: 순서 / 질문 의도 / 짧은 검색 질문. 현재 분석을 보완할 질문을 반드시 2~3개만 추천
...
- "추가로 찾을 표"에는 서로 다른 표를 찾는 질문 2~3개를 제안한다. "질문 의도"는 현재 분석에서 무엇을 판단하거나 확인하려는지 짧은 목적 문구로 쓰고, 조회할 컬럼을 반복해서 나열하지 않는다.
- 각 행의 "짧은 검색 질문"은 질문 의도와 구분되게 실제 SQL 조회에 바로 사용할 수 있는 한 문장으로 쓰며, 한 번에 표 하나만 조회한다(권장 20~45자).
- 검색 질문에는 필요한 기간·구분 기준·핵심 지표만 넣고 원인 분석, 검증 절차, 여러 후속 작업을 한 문장에 합치지 않는다. 예: 질문 의도 "제품별 수익성 변화 확인" / 검색 질문 "2026년 월별 제품별 매출과 매출원가 보여줘", 질문 의도 "비용 급증 계정 식별" / 검색 질문 "2026년 6~7월 계정별 판관비 보여줘".
- 검색 질문의 테이블·컬럼 표현은 database_schema_blueprint, source_tables, columns.name에서 확인되는 용어를 우선 사용하고 없는 업무 용어를 만들지 않는다.
- 각 검색 질문을 실행해 결과 표 2~3개를 저장한 뒤 다시 AI 분석에 함께 넣을 수 있도록 추천한다.
- 표 제목은 자연스러운 한국어로 쓰고 EVIDENCE_JSON의 영문 키를 그대로 노출하지 않는다.
- 숫자는 천 단위 구분과 적절한 단위를 사용하되 원래 값의 의미를 바꾸지 않는다.
- "추가로 찾을 표"는 3개 열과 2~3개 행으로 제한하고 긴 설명을 넣지 않는다. 나머지 표는 3~6개 열과 3~8개 행으로 제한하며, 긴 설명은 "근거와 해석"처럼 폭이 넓은 마지막 열에 배치한다.
